...
## 2026-09-21：14 种收藏卡材质上线

生产部署 dpl_giix8nJBs8A1SKgXQRmihaLLwHcf，别名 https://blueprint-card-v3-preview.vercel.app 。
- 首页图鉴、正式卡面、PNG/分享合成统一使用已确认的 cards-css 参数；旧材质评审和 motion 路径转到 /debug/foil。
- 活跃抽卡版本 v0.2.0：14 种材质等权重，版型独立为 classic 98%、aura 1%、animation 1%；首抽同样适用。
- 服务端 HMAC 的独立 128 位区间决定两项结果；生图提示词遵循服务端结果；新生图请求拒绝过期概率版本，已存在的任务仍可恢复。
- 271 项测试、生产构建、14 材质交互与完整模拟生图流程通过。导出使用真实 CSS 材质快照；离屏图片设为 eager，避免惰性加载卡住导出。
- 当前线上 OPENAI_API_KEY 尚未配置（更新前即如此）；本次没有付费生图调用，也未宣称真实 AI 生图可用。
